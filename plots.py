...
#pm.powerplants(from_url=False, update=True)
import numpy as np
import matplotlib.pyplot as plt
import seaborn as sns
import pandas as pd
from powerplantmatching.core import get_config
from powerplantmatching.data import (CARMA, ENTSOE, GEO, OPSD, JRC, WEPP,
                                     GPD)
...
df_dict = {0: 'CARMA', 1: 'ENTSOE', 2: 'JRC',3: 'GEO', 4: 'OPSD', 5: 'WEPP'}
...
# Prepare
dfs = [CARMA(), ENTSOE(), JRC(), GEO(), GPD(filter_other_dbs=False), OPSD(),
       pm.collection.matched_data()]
dfs = [df[lambda df: (~df.Fueltype.isin(excluded_fueltypes))] for df in dfs]
keys = ['CARMA', 'ENTSOE', 'JRC', 'GEO', 'GPD', 'OPSD', 'Matched Data']
...
# %%
m = pm.collection.Carma_ENTSOE_JRC_GEO_IWPDCY_OPSD_WEPP_matched_reduced()
m = m[~m.Fueltype.isin(excluded_fueltypes)]
...
# Prepare
opsd = OPSD()
df_dict = {0: 'CARMA', 1: 'ENTSOE', 2: 'JRC', 3: 'GEO', 4: 'OPSD',
           5: 'WEPP'}
for i, name in df_dict.items():
    dfp = (locals()[str.lower(name)]
           .loc[lambda x: ~x.Fueltype.isin(excluded_fueltypes)])
...
df = pm.collection.Carma_ENTSOE_JRC_GEO_IWPDCY_OPSD_WEPP_matched_reduced()
df['matches'] = df['projectID'].apply(len)
df['matches'] += 0.4 * (2.*np.random.rand(len(df)) - 1.)

projects = ['Matched w/ WEPP', 'OPSD', 'CARMA', 'ENTSOE', 'JRC', 'GEO',
            'WEPP']
...
df_w_wepp = pm.collection.\
    Carma_ENTSOE_JRC_GEO_IWPDCY_OPSD_WEPP_WRI_matched_reduced()
    #    Carma_ENTSOE_JRC_GEO_IWPDCY_OPSD_WEPP_WRI_matched_reduced()
...
m_with_wepp = pm.collection.\
    Carma_ENTSOE_JRC_GEO_IWPDCY_OPSD_WEPP_matched_reduced()
