...
# Mind Upper/Lower case writing
    ...
    return (pd.concat([opsd_DE, opsd_EU], ignore_index=True)
...
def Capacity_stats(raw=False, level=2, config=None, update=True,
...
def WEPP(raw=False, config=None):
    ...
    wepp = pd.read_csv(config['WEPP']['fn'], dtype=datatypes,
                       encoding='Windows-1252', engine='python', error_bad_lines=False, warn_bad_lines = True)
    if raw:
        return wepp
    ...
    wepp.rename(columns={'Unit': 'Name',
                         ...
                         'Lon': 'lon',
                         'Unitid': 'projectID',
                         'Electype':'Electype_AUP'
                         }, inplace=True)
    #wepp=wepp.loc[wepp['Electype_AUP']=='A']
...
def UBA(header=9, skipfooter=26, prune_wind=False, prune_solar=False,
    ...
    """
    config = get_config() if config is None else config
    default_url = get_config(_package_data('config.yaml'))['UBA']['url']
    url = get_config(_package_data('config.yaml'))['UBA']['url']
    #ppm.core.get_config('config.yaml')['UBA']['url']
    err = IOError(f'The URL seems to be outdated, please copy the new url '
                  f'\n\n\t{default_url}\n\nin your custom config file '
                  f'\n\n\t{package_config["custom_config"]}\n\nunder tag "UBA" '
                  '-> "fn"')
    def parse_func():
        parse = requests.get(url)
        if parse.ok:
            content = pd.read_excel(url, skipfooter=skipfooter,sheet_name='Kraftwerke',na_values='n.b.', header=header)
        else:
            raise(err)
        return content
...
def BNETZA(header=9, sheet_name='Gesamtkraftwerksliste BNetzA',
    ...
    config = get_config() if config is None else config
    #default_url = get_config(_package_data('config.yaml'))['BNETZA']['url']
    #url = ppm.core.get_config('config.yaml')['BNETZA']['url']
   
    url='https://www.bundesnetzagentur.de/SharedDocs/Downloads/DE/Sachgebiete/Energie/Unternehmen_Institutionen/Versorgungssicherheit/Erzeugungskapazitaeten/Kraftwerksliste/Kraftwerksliste_2019_1.xlsx;jsessionid=4A3E2CA19EB8CBA914A53884B6ADC51A?__blob=publicationFile&v=2'
    ...
    def parse_func():
        parse = requests.get(url)
        if parse.ok:
            content = pd.read_excel(url, sheet_name=sheet_name, header=header)
        else:
            raise(err)
        return content
