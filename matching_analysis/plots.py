...
from powerplantmatching.data import (CARMA, ENTSOE, GEO, OPSD, WEPP,
                                     GPD)
...
df_dict = {0: 'CARMA', 1: 'ENTSOE', 2: 'GEO', 3: 'OPSD', 4: 'WEPP'}
y_labels = []
x_labels = []
...
# Prepare
dfs = [CARMA(), ENTSOE(), GEO(), WEPP(), GPD(filter_other_dbs=False), OPSD(), pm.collection.matched_data(reduced=True)]
dfs = [df[lambda df: (~df.Fueltype.isin(excluded_fueltypes))] for df in dfs]
keys = ['CARMA', 'ENTSOE', 'GEO', 'WEPP','GPD','OPSD', 'Matched Data']
...
# %% Figure 5.5: Bar_decommissioning curve.
fig, ax = pm.plot.bar_decomissioning_curves(df)


# %% Figure 6
fig, ax = pm.plot.comparison_1dim(include_WEPP=True, by='Fueltype',
                                  axes_style='darkgrid',
                                  figsize=(figwidth, 4.),
                                  exclude=excluded_fueltypes)
ax.legend(framealpha=0.5)
ax.set_facecolor('lavender')
fig.tight_layout(pad=0.2)
fig.savefig('stats_matched_fueltype_comparison_w_wepp.png', dpi=300)


# %% Figure 7

fig, ax = pm.plot.comparison_1dim(figsize=(figwidth, 5.5),
                                  include_WEPP=True, axes_style='darkgrid',
                                  exclude=excluded_fueltypes)
ax.legend(framealpha=0.5)
ax.set_facecolor('lavender')
fig.tight_layout(pad=0.2)
...
# Prepare
opsd = OPSD()
df_dict = {0: 'CARMA', 1: 'ENTSOE', 2: 'GEO', 3: 'OPSD',
            4: 'WEPP'}
for i, name in df_dict.items():
...
df = pm.collection.matched_data()
df['matches'] = df['projectID'].apply(len)
df['matches'] += 0.4 * (2.*np.random.rand(len(df)) - 1.)

projects = ['Matched w/ WEPP', 'OPSD', 'CARMA', 'ENTSOE', 'GEO', 'WEPP']
...
df_w_wepp = pm.collection.matched_data()
df_wo_wepp = pm.collection.matched_data(pm.data.get_config(matching_sources=('CARMA', 'ENTSOE', 'GEO', 'GPD', 'OPSD' )))    
...
lu = pm.utils.lookup([df_w_wepp, df_wo_wepp, stats],
...
fig, ax = pm.plot.comparison_countries_fueltypes_bar(
        include_WEPP=True, include_VRE=False, show_indicators=False, year=2016,
