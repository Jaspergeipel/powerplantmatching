...
def fueltype_totals_bar(dfs, keys=None, figsize=(7, 4), unit='GW',
    ...
    with sns.axes_style(axes_style):
        ...
        if last_as_marker:
            as_marker = dfs[-1]
            #print('asmarker=', as_marker)
            as_marker_key = keys[-1]
            #print('as_marker_key=', as_marker_key)
            df_rest = dfs[:-1]
            #print('df:rest=',  df_rest)
            rest_keys = keys[:-1]
            #print('rest_keys=', rest_keys)
        fueltotals = lookup(df_rest, keys=rest_keys, by='Fueltype', unit=unit)
        fueltotals.sort_index(axis = 0) 
        fueltotals.plot(kind="bar", ax=ax, edgecolor='none', rot=90, **kwargs)
        #print(fueltotals)
        if last_as_marker:
            fueltotals = lookup(as_marker, keys=as_marker_key, by='Fueltype', unit=unit)
            fueltotals.sort_index(axis = 0)
            fueltotals.plot(kind="line", ax=ax, label=as_marker_key, markeredgecolor='none',
...
def comparison_countries_fueltypes_bar(
        ...
        show_indicators=True, year=2016, **kwargs):
    ...
    labels_mpatches = OrderedDict(sorted(labels_mpatches.items()))
...
def matched_fueltype_totals_bar(figsize=(9, 4), axes_style='whitegrid'):
    ...
    matched = set_uncommon_fueltypes_to_other(pm.collection.matched_data(pm.data.get_config(matching_sources=('CARMA', 'ENTSOE', 'GEO', 'GPD', 'OPSD' ))))
...
def bar_decomissioning_curves(df=None, ylabel=None, title=None,
    ...
    if ylabel is None:
        ylabel = 'Capacity [GW]'
    if df is None:
        df = pm.collection.matched_data(pm.data.get_config(matching_sources=('CARMA', 'ENTSOE', 'GEO', 'GPD', 'OPSD', 'WEPP' )))
    ...
    allowed_countries = ('Austria','Germany', 'France', 'United Kingdom', 'Spain', 'Italy')
    df=df.loc[lambda x: x.Country.isin(allowed_countries)]
    df = df.copy()

    df.loc[:, 'Life'] = df.Fueltype.map(get_config()['fuel_to_color'])

    # Insert periodwise capacities
    df.loc[:, 2016] = df.loc[:, 'Capacity']
    ...
    nrows, ncols, rem = gather_nrows_ncols(len(set(df.Country)))
...
def area_yearcommissioned(dfs, keys, figsize=(7, 5),
    ...
    labels_mpatches = OrderedDict(sorted(labels_mpatches.items()))
    fig.legend(labels_mpatches.values(), labels_mpatches.keys(),
              loc=8, ncol=int(len(labels_mpatches)/2), facecolor='#d9d9d9',
              prop={'size': 10})
    return fig, ax
...
def comparison_single_matched_bar(df=None, include_WEPP=True, cleaned=True,
    ...
    if df is None:
        if include_WEPP:
            df = (pm.collection.matched_data(pm.data.get_config(matching_sources=('CARMA', 'ENTSOE', 'GEO', 'GPD', 'OPSD', 'WEPP' ))).loc[lambda x:  ~x.Fueltype.isin(exclude)])
                #Carma_ENTSOE_ESE_GEO_GPD_IWPDCY_OPSD_WEPP_matched_reduced()
        else:
            df = (pm.collection.matched_data(pm.data.get_config(matching_sources=('CARMA', 'ENTSOE', 'GEO', 'GPD', 'OPSD' ))).loc[lambda x:  ~x.Fueltype.isin(exclude)])
                #Carma_ENTSOE_ESE_GEO_GPD_IWPDCY_OPSD_matched_reduced()
    if cleaned:
...
#                              use_saved_aggregation=use_saved_aggregation)
    ...
    if include_WEPP:
        #removed WRI/ESE
        stats = lookup(df=[carma, entsoe, geo, opsd, wepp],
                      keys=['CARMA', 'ENTSO-E', 'GEO', 'OPSD', 'WEPP'],
                      exclude=exclude, by='Fueltype')/1000
    else:
        stats = lookup(df=[carma, entsoe, geo, opsd],
                      keys=['CARMA', 'ENTSO-E', 'GEO', 'OPSD'],
                      exclude=exclude, by='Fueltype')/1000
...
def comparison_1dim(dfs=None, keys=None, by='Country', include_WEPP=True,
    ...
    if dfs is None and keys is None:
        ...
        dfs = [df for df in gather_comparison_data(include_WEPP=include_WEPP, include_VRE=include_VRE, year=year)
              if df is not None]
        if include_WEPP:
    ...
    #print(stats)
    #stats=stats.loc[lambda x: x.index.isin(('Austria','Germany', 'France', 'United Kingdom', 'Spain', 'Italy'))]
    font = {'size': 12}
    plt.rc('font', **font)
    if how == 'hbar':
        with sns.axes_style(axes_style):
...
def gather_comparison_data(include_WEPP=True, include_VRE=False, year=2016, **kwargs):
    ...
    if include_WEPP:
        ...
        #if include_VRE:
         #   red_w_wepp =pm.collection.matched_data(pm.data.get_config(matching_sources=('CARMA', 'ENTSOE', 'GEO', 'GPD', 'OPSD', 'WEPP' )))
            #\
            #    Carma_ENTSOE_ESE_GEO_GPD_IWPDCY_OPSD_WEPP_matched_reduced_VRE()
        #else:
        red_w_wepp =pm.collection.matched_data(pm.data.get_config(matching_sources=('CARMA', 'ENTSOE', 'GEO', 'GPD', 'OPSD', 'WEPP')))
        ...
        red_w_wepp=red_w_wepp.loc[lambda x: x.Country.isin(allowed_countries)]
        #wepp.query(queryexpr, inplace=True)
        wepp=wepp.loc[lambda x: x.Country.isin(allowed_countries)]
    else:
        wepp = None
        red_w_wepp = None
    # 3: Reduced w/o WEPP
    #if include_VRE:
    #    red_wo_wepp =pm.collection.matched_data(pm.data.get_config(matching_sources=('CARMA', 'ENTSOE', 'GEO', 'GPD', 'OPSD' )))
    ...
    red_wo_wepp=red_wo_wepp.loc[lambda x: x.Country.isin(allowed_countries)]
    # 4: Statistics
    ...
    statistics=statistics.loc[lambda x: x.Country.isin(allowed_countries)]
